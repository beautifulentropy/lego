...
func NewChallenge(core *api.Core, validate ValidateFunc, opts ...ChallengeOption) (*Challenge, error) {
	...
	if chlg.accountURI == "" {
		return nil, errors.New("dnspersist01: ACME account URI is required")
	}
...
func (c *Challenge) Solve(authz acme.Authorization) error {
	...
	log.Infof("[%s] acme: Checking DNS-PERSIST-01 record propagation. [nameservers=%s]", domain, strings.Join(recursiveNameservers, ","))
	time.Sleep(interval)

	err = wait.For("propagation", timeout, interval, func() (bool, error) {
		ok, err := c.preCheck.call(domain, fqdn, matcher, c.checkDNSPropagation)
		if !ok || err != nil {
			log.Infof("[%s] acme: Waiting for DNS-PERSIST-01 record propagation.", domain)
		}
