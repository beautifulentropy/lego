...
func (d *DNSProvider) Present(domain, token, keyAuth string) error {
	...
	zones, err := d.client.GetZones(ctx, &nodion.ZonesFilter{Name: dnsrecord.UnFqdn(authZone)})
	if err != nil {
		return fmt.Errorf("nodion: %w", err)
	}
...
func (d *DNSProvider) CleanUp(domain, token, keyAuth string) error {
	...
	if err != nil {
		return fmt.Errorf("regru: failed to remove TXT records [domain: %s]: %w", dnsrecord.UnFqdn(authZone), err)
	}
